// app.js
// Связка UI ↔ DataLoader ↔ GRUClassifier. EDA, обучение, подбор порога, экспорт предсказаний, аугментация позитивов.
...
export class App {
  constructor({ tf, Chart, dl, model, ui }) {
    ...
    this.lastPreds = null; // для экспорта
    ...
    try {
      ...
      this.ui.edaText.innerHTML =
        `Баланс классов — Yes: <b>${balance.positive}</b>, No: <b>${balance.negative}</b> (доля ${(balance.rate*100).toFixed(1)}%).<br/>
         Топ-числовые корреляции с Attrition: <b>${topCorr.map(([k,v])=>`${k} (${v.toFixed(2)})`).join(', ')}</b>.<br/>
         OverTime явно влияет (см. график).`;
  ...
  async #prepare() {
    try {
      ...
      // собрать конфиг аугментации из UI
      const augment = {
      ...
      this.#log(`Dataset ready. Augment=${augment.enable?'on':'off'}; Timesteps: ${this.dataset.xTrain.shape[1]}, Features: ${this.dataset.xTrain.shape[2]}`);
    } catch (e) {
      alert(e.message || String(e));
    }
  }
  ...
  async #evaluate() {
    try {
      ...
      // подготовим предсказания для экспорта
